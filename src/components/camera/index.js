...
function Camera({ addImage, shutterEnabled }) {
  ...
  return (
    <div className="Camera__wrapper">
      {hasCamera === null && <dialog open>Accessing camera...'</dialog>}
      {hasCamera === false ? (
        <dialog className="Camera__dialogbox" open>
          <Modal>
            <button onClick={() => setHasCamera(null)}>Check again</button>
          </Modal>
        </dialog>
      ) : (
          ...
          {(hasCamera || hasCamera === null) && (
            <>
              <Countdown
                seconds={3}
                callback={grabImage}
                isEnabled={shutterEnabled && hasCamera}
                checkHasCamera={checkHasCamera}
              />
              <Webcam
